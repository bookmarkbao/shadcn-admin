...
1. 确保 API Server 已启用。
2. 访问 `http://127.0.0.1:23333/api/db/user_expressions?_limit=5`。
3. 任何成功响应都返回：
   ```json
   {
     "success": 1,
...
| `column_like=keyword` | 模糊匹配（自动包裹 `%`）                      | `expression_like=That makes sense`               |
| `_order=col:asc`      | 排序，多字段用逗号分隔                        | `_order=added_at:desc,expression:asc` |
